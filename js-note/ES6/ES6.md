...
### let命令；声明变量的技巧；避免变量提升；具有暂时性死区的特性（没有用let声明之前处于暂时性死区状态，用typeof声明会报错，没let之前会报undefined）；不允许重复声明
...
+ 模板字符串，是增强版的字符串，用反引号（`）标识  
	...
	> 模板字符串甚至还能嵌套  
	![模板字符串甚至还能嵌套](./lib/img/String/ES6嵌套字符串模板.png "模板字符串甚至还能嵌套")  
	> 如果需要引用模板字符串本身，在需要时执行，可以写成函数  
	> 模板字符串的应用实例：模板编译  
	![模板编译1](./lib/img/String/模板编译1.png "模板编译1")  
	![模板编译2](./lib/img/String/模板编译2.png "模板编译2")  
	![模板编译3](./lib/img/String/模板编译3.png "模板编译3") 
...
+ `String.replaceAll()`，历史上，字符串的实例方法replace()只能替换第一个匹配，ES2021 引入了replaceAll()方法，可以一次性替换所有匹配
	...
	![replaceAll第二个参数可以使用一些特殊字符串](./lib/img/String/replaceAll第二个参数可以使用一些特殊字符串.png "replaceAll第二个参数可以使用一些特殊字符串")
...
+ `Object.is()`，用来比较两个值是否完全相等
	...
	![js中双等号存在的类型转换问题](./lib/img/Object/js中双等号存在的类型转换问题.png "js中双等号存在的类型转换问题")  
...
+ `Object.entries()`，返回一个数组，成员是参数对象自身的（不含继承的）所有可遍历（enumerable）属性的键值对数组（类似`[[k1,v1],[k2.v2]]`）
	> 除了返回值不一样，该方法的行为与`values()`基本一致  
	> `entries()`基本用途是遍历对象的属性，***！！！这里注意！！！***  
...
### Set 的遍历操作
+ `Set.keys()`方法，由于`Set`结构没有键名，只有键值（或者说键名和键值是同一个值），所以`keys()`方法和`values()`方法的行为完全一致  
+ `Set.values()`方法，`Set`结构的实例默认可遍历，它的默认遍历器生成函数就是它的`values()`方法，这意味着可以省略`values()`方法，直接用`for...of`循环遍历`Set`  
+ `Set.entries()`方法，`entries()`方法返回的遍历器，同时包括键名和键值，所以每次输出一个数组，它的两个成员完全相等  
+ `Set.forEach()`方法，`Set`结构的实例与数组一样，也拥有`forEach()`方法，用于对每个成员执行某种操作，没有返回值，`forEach()`方法的参数就是一个处理函数，该函数的参数与数组的`forEach()`一致，依次为键值、键名、集合本身  
+ 这里需要注意，`Set`结构的键名就是键值（两者是同一个值），因此第一个参数与第二个参数的值永远都是一样的  
+ 值得一提的是，`Set`的遍历顺序就是插入顺序，这个特性有时候非常有用，比如使用`Set`保存一个回调函数列表，调用时就能保证按照添加顺序调用  
+ 注意对象里命名函数不可以用箭头函数！！！
+ 相关参考资料  
	1. [Symbol.iterator 参考资料](https://blog.csdn.net/margin_0px/article/details/82971545)，后期详细记录学习  
	2. [ES6 for...of 循环 参考资料](https://blog.csdn.net/ixygj197875/article/details/79188258)，后期详细记录学习  
	3. [另外，forEach方法还可以有第二个参数，表示绑定处理函数内部的this对象 参考资料](https://www.icode9.com/content-1-242023.html)  
	4. [JS forEach()第二个参数 参考资料一](https://juejin.cn/post/6844903777133133838)  
	5. [JS forEach()第二个参数 参考资料二](https://blog.csdn.net/weixin_30782293/article/details/96337127)


#### =============================================================
# 20210622 应该是继续从这里学习，call/apply再次重点学习理解
### 遍历的应用
#### =============================================================
...
#### 目前学习所用时间：11小时左右（之前在嘉科的时候还有闲心思记录学习时间哈哈哈，从这里开始后面暂时不记录了，碎片化学习抽空看看）